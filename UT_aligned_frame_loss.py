#run this code for frame loss experiment
...
# (p * number of frames) will be dropped in each iteration
p = 0.1
#number of iterarions 
q = 4

#dropping frames from all samples 
for i in range(len(all_action_prepared)):
    for j in range(len(all_action_prepared[i])):
        for w in range(q):
            a = (int(random.uniform(0,len(all_action_prepared[i][j]))-len(all_action_prepared[i][j])*p))
            del all_action_prepared[i][j][a:(a+int(len(all_action_prepared[i][j])*p))]
            
#dropping frames from reference samples 
for i in range(len(ref_samples)):
    for j in range(len(ref_samples[i])):
        for w in range(q):
            a = (int(random.uniform(0,len(ref_samples[i][j]))-len(ref_samples[i][j])*p))
            del ref_samples[i][j][a:(a+int(len(ref_samples[i][j])*p))]
